...
// Mock line chart using SVG — matches the Route Rate Limit Traffic chart
function MockLineChart() {
  const w = 600, h = 120;
  const points = [32, 33, 31, 30, 31, 58, 65, 70, 93, 70, 97, 8];
  const times  = ['16:50','16:51','16:52','16:53','16:54','16:55','16:56','16:57','16:58','16:59','17:00','17:01'];
  const max    = 110;
  ...
  return (
    <div className="relative">
      ...
      <div className="px-4 py-3 border-b border-white/5">
        <p className="text-xs font-medium text-white">Route Rate Limit Traffic</p>
        <p className="text-[10px] text-zinc-500 mt-0.5">Incoming request traffic compared against the configured route rate limit</p>
      </div>
        ...
        <svg viewBox={`0 0 ${w} ${h + 20}`} className="w-full" style={{ height: 130 }}>
          ...
          {[0, 25, 50, 75, 100].map(v => (
            <g key={v}>
              ...
              />
              <text x={-2} y={toY(v) + 4} textAnchor="end" fontSize={8} fill="rgba(161,161,170,0.5)">{v}</text>
            </g>
          ))}

          {/* Rate limit dashed line */}
          <line
            x1={0} y1={rateLimitY} x2={w} y2={rateLimitY}
            stroke="rgba(239,68,68,0.55)" strokeWidth={1.5} strokeDasharray="5 4"
          />
          <text x={w - 2} y={rateLimitY - 4} textAnchor="end" fontSize={8} fill="rgba(239,68,68,0.8)">Rate Limit: 50</text>
          ...
          {times.map((t, i) => (
            i % 2 === 0 && (
              <text key={i} x={toX(i)} y={h + 16} textAnchor="middle" fontSize={8} fill="rgba(161,161,170,0.5)">{t}</text>
            )
          ))}
        </svg>
      </div>
...
function MockStats() {
  const stats = [
    { label: 'Total Requests', value: '5,605', icon: '⇄', color: 'text-zinc-400' },
    { label: 'Allowed Requests', value: '3,483', icon: '✓', color: 'text-emerald-400' },
    { label: 'Blocked Requests', value: '2,092', icon: '⚠', color: 'text-amber-400' },
    { label: 'Avg Latency', value: '681ms', icon: '◷', color: 'text-zinc-400' },
  ];

  return (
    <div className="grid grid-cols-4 gap-2 px-4 py-3 border-t border-white/5">
      {stats.map((s) => (
        <div key={s.label} className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02] border border-white/5">
          <div className={`w-6 h-6 rounded-md bg-white/5 flex items-center justify-center text-[10px] ${s.color}`}>
            {s.icon}
          </div>
          <div>
...
// Mock horizontal bar chart — matches Load Balancer Distribution
function MockLoadBalancer() {
  const backends = [
    { url: 'http://127.0.0.1:9001', pct: 96, state: 'healthy' },
    { url: 'http://127.0.0.1:9000', pct: 84, state: 'healthy' },
    { url: 'http://127.0.0.1:9002', pct: 6,  state: 'half_open' },
  ];

  const color = (state) =>
    state === 'open' ? 'from-red-500/30 to-red-500/80'
    : state === 'half_open' ? 'from-amber-500/30 to-amber-500/80'
    : 'from-emerald-500/30 to-emerald-500/80';

  return (
    <div className="border-t border-white/5">
      <div className="px-4 py-3 border-b border-white/5">
        <p className="text-xs font-medium text-white">Load Balancer Distribution</p>
        <p className="text-[10px] text-zinc-500 mt-0.5">Traffic distribution across active route backends</p>
      </div>

      <div className="px-4 py-4 space-y-3">
        {backends.map((b) => (
          <div key={b.url} className="flex items-center gap-3">
            <span className="text-[10px] text-zinc-500 font-mono w-36 flex-shrink-0 text-right">{b.url}</span>
      ...
      <div className="px-4 pb-3 flex items-center gap-4">
        {[
          { color: 'bg-emerald-400', label: 'Healthy (Closed)' },
          { color: 'bg-amber-400',   label: 'Degraded (Half Open)' },
          { color: 'bg-red-400',     label: 'Offline (Open)' },
        ].map(l => (
            ...
            <div className="relative rounded-xl border border-border bg-[#0a0a0a]/80 backdrop-blur-sm overflow-hidden animate-glow text-left">
              ...
              <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-white/[0.02]">
                ...
                <div className="flex-1 text-center">
                  <span className="text-xs text-zinc-500 font-mono">strata.dev / monitoring</span>
                </div>
                {/* Fake status pill */}
